...
function wolf_core_output_license_tab() {
	?>
	<a href="#license" class="nav-tab"><?php esc_html_e( 'License', '%TEXTDOMAIN%' ); ?></a>
	<?php
}
...
function wolf_core_output_license_tab_content() {
	...
	<?php
		...
		<ul class="wvc-license-info">
			...
			<?php
				echo sprintf(
					wp_kses_post( __( '%s theme works with <strong>%s</strong> plugin to offer all its features.', '%TEXTDOMAIN%' ) ),
					$theme_name,
			...
			<li>
				<?php
					echo sprintf(
						wp_kses_post( __( 'It extends of <a href="%s" target="_blank">%s</a> and <a href="%s" target="_blank">%s</a> plugin.', '%TEXTDOMAIN%' ) ),
			...
			<li>
				<?php esc_html_e( 'It includes plugin territory features that boost the theme functionalities.', '%TEXTDOMAIN%' ); ?>
			</li>
			<li>
				<?php
				echo sprintf(
					wp_kses_post( __( 'This extension is available only to users who purchased their theme from <a href="%s" target="_blank">%s</a>.', '%TEXTDOMAIN%' ) ),
			...
			<?php
				// echo sprintf(
				// 	wp_kses_post( __( 'You <strong>do not need to activate %s</strong> as the full version is already included in the theme (<a href="%s" target="_blank">more infos</a>).', '%TEXTDOMAIN%' ) ),
				// 	'WPBakery Page Builder',
		...
		<p class="wvc-license-cta-text">
			<?php
				echo sprintf(
					wp_kses_post( __( 'Please enter your <strong>theme purchase code</strong> below to activate your theme license and be able to use all features.', '%TEXTDOMAIN%' ) ),
					'WolfThemes'
				);
			?>
		</p>
		<form method="post" action="<?php echo esc_url( admin_url( 'themes.php?page=' . $theme_slug . '-about' ) ); ?>">
		<input name="theme_purchase_code" placeholder="693e0017-48d3-4bd5-be47-1c5c14e7ab9c" type="text" class="regular-text wvc-license-input"><input value="<?php esc_html_e( 'Activate', '%TEXTDOMAIN%' ); ?>" type="submit" class="button button-primary wvc-license-button">
		</form>
		<p>
			<a target="_blank" href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Can-I-Find-my-Purchase-Code-"><?php esc_html_e( 'How to find your purchase code', '%TEXTDOMAIN%' ); ?></a>
		</p>
		<?php else : ?>
		<p><?php
			echo sprintf(
				wp_kses_post( __( 'The %s is activated.', '%TEXTDOMAIN%' ) ),
				'Wolf Core'
			);
		?></p>
		<form method="post" action="<?php echo esc_url( admin_url( 'themes.php?page=' . $theme_slug . '-about' ) ); ?>"><input name="wolf_core_reset_purchase_code" value="<?php esc_html_e( 'Reset purchase code', '%TEXTDOMAIN%' ); ?>" type="submit" class="button button-secondary">
			</form>
		<?php
		endif;
...
function wolf_core_activate_theme() {
	...
	if ( ! $activated && isset( $_POST['theme_purchase_code'] ) ) {
		...
		if ( isset( $_POST['theme_purchase_code'] ) && ! empty( $_POST['theme_purchase_code'] ) ) {
			...
			if ( ! is_wp_error( $response ) && is_array( $response ) ) {
				...
				if ( $body ) {
					...
					if ( $data && is_object( $data ) && isset( $data->code ) && isset( $data->key ) ) {
						...
						echo '<p>';
						esc_html_e( 'Extension activated', '%TEXTDOMAIN%' );
						echo '</p>';
						echo '</div>';
			...
			$is_error = true;
			$error = esc_html__( 'Purchase code can not be empty', '%TEXTDOMAIN%' );
		}
