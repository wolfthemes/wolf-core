<?php
/**
 * Icon with text shortcode template (deprecated)
 *
 * @author WolfThemes
 * @category Core
...
if ( 'left_from_title' == $position ) {
		...
		$output .= wp_kses(
			...
			array(
				'i'   => array(
					'class'                 => array(),
					'style'                 => array(),
					'data-icon-hover-color' => array(),
				),
				'div' => array(
					'class'                   => array(),
					'style'                   => array(),
					'data-bg-hover-color'     => array(),
					'data-border-hover-color' => array(),
				),
				'h1'  => array(
					'class' => array(),
					'style' => array(),
	...
	if ( $text ) {
		$output .= '<p style="' . wvc_esc_style_attr( $p_style ) . '">' . $text . '</p>';
	}
