...
if ( ! function_exists( 'wolf_theme_is_activated' ) || ! wolf_theme_is_activated() ) {
	return;
}

class Elementor_Buy_Button_Widget extends \Elementor\Widget_Base { // phpcs:ignore
	...
	public function __construct( $data = array(), $args = null ) { // phpcs:ignore
		...
		$this->params = wolf_core_buy_button_params();
	...
	protected function render() {
		...
		if ( ! empty( $atts['product_id'] ) ) {
			...
			if ( ! empty( $atts['variation_id'] ) ) {
				$url_args['variation_id'] = absint( $atts['variation_id'] );
			}

			if ( ! empty( $atts['quantity'] ) && absint( $atts['quantity'] ) > 1 ) {
				$url_args['quantity'] = absint( $atts['quantity'] );
			}

			if ( ! empty( $atts['redirect_to_checkout'] ) && 'yes' === $atts['redirect_to_checkout'] ) {
				$url_args['redirect_to_checkout'] = 1;
			}
...
\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new \Elementor_Buy_Button_Widget() );